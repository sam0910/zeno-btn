...
class MQTTClient(MQTT_base):
    def __init__(self, config):
        ...
        self._has_connected = True  # Define 'Clean Session' value to use.
        self._tasks = []
        if ESP8266:
            import esp
    ...
    async def connect(self, *, quick=True):  # Quick initial connect option for battery apps
        ...
        if not self._events:
            self.loop.create_task(self._wifi_handler(True))  # User handler.
        if not self._has_connected:
            self._has_connected = True  # Use normal clean flag on reconnect.
            self.loop.create_task(self._keep_connected())
            # Runs forever unless user issues .disconnect()

        self.loop.create_task(self._handle_msg())  # Task quits on connection fail.
        self._tasks.append(self.loop.create_task(self._keep_alive()))
        if self.DEBUG:
            self._tasks.append(self.loop.create_task(self._memory()))
        if self._events:
            self.up.set()  # Connectivity is up
        else:
            self.loop.create_task(self._connect_handler(self))  # User handler.
    ...
    def _reconnect(self):  # Schedule a reconnection if not underway.
        ...
        if self._isconnected:
            ...
            self.loop.create_task(self._kill_tasks(True))  # Shut down tasks and socket
            if self._events:  # Signal an outage
                self.down.set()
            else:
                self.loop.create_task(self._wifi_handler(False))  # User handler.
